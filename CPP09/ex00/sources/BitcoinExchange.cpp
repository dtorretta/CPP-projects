...
/*la validación siempre se hace antes de asignar la clave (key) y el valor (value) en el mapa.*/
...
void BitcoinExchange::processDatabase()
{
    ...
    while (std::getline(_file, line)) 
    {
        ...
        _database[date] = floatValue;  //Asocia un XR (value) para un determinado Date (Key)
    }
}
...
void BitcoinExchange::processBitcoin(char *file)
{
    ...
    while (std::getline(dataFile, line))
    {
        ...
        {        
            ...
            float floatValue = atof(valuePart.c_str()); //c_str() convierte std::string a const char*, que es lo que esperan atof.
