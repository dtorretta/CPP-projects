...
std::vector<Pairs> divideIntoPairs(std::vector<int> vector, int &odd)
{
    ...
    for(it = vector.begin(); it < vector.end() - 1; ++it) //-1 porque b mira el siguiente
    {
        int a = *it;
        ++it;
        int b = *it;
        ...
        Pairs p;  //creo el nuevo nodulo que formara parte del vector pair 
        p.L = a;
        p.S = b;
        pairs.push_back(p);
    }
...
{   
    ...
    std::vector<Pairs> pairs;
    int odd = -1;
    pairs = divideIntoPairs(vector, odd); //la funcion espera una referencia asi que en vez de mandar un pointer, mando el int, y las modificaciones sera visibles
